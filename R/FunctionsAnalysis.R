...
AddControl<-function(ControlDB, PatientsDB, id, bed, c, include.chr, dirPrinc){
  ...
  if (!(id %in% colnames(ControlDB))){ #Make sure the patient is not already in controlDB
    ...
    if(id %in% colnames(PatientsDB)){ #If the patients is in PatientsDB-----------------------------
      ...
      if(substr(path_file,nchar(path_file)-3, nchar(path_file)) == ".bam"){ #If its .BAM
        ...
        bam.control<-BAMfromFastQ(dirPrinc)
...
AddPatient<-function(ControlDB, PatientsDB, id, bed, p, include.chr, dirPrinc){
...
AnalyzePatient<-function(ControlDB, PatientsDB, id, bed, p, include.chr, dirPrinc){
  #id<-readline(prompt = "Patient ID? ")
...
Report<- function(CNV_calls, graph, id, dir_patient){
...
BAMfromFastQ <- function(dirPrinc, keepFastQ = TRUE){
