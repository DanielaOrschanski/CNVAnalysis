...
if(!("MitoRSoftware" %in% existingFiles("~"))){
  ...
  if(d=="Y"|d=="y"){ InstallationsforFASTQ() #PREGUNTAR A JUAN!
  }else{ stop() }
}


#Como hacemos para que se escriba completo el path? Sys.getenv("R_LIBS_USER")
...
if(TYPE=="N"| TYPE== "n"){
  ...
  bed<-readRDS("~/CNVAnalisisDefinitivo/CNVAnalisis-Package/CNVAnalysis/Data-CNV/bedfileHuman.RDS")
...
#----------------------------------------------------------------------
    ...
    if(!(file.exists("~/DataBases/ControlMitoDB.RDS"))){
      InitialDB(c, p, e)
      message("Data Bases have been created. You can find them in DataBases folder")
    }
    ...
    bed<-readRDS("~/CNVAnalisisDefinitivo/CNVAnalisis-Package/CNVAnalysis/Data-CNV/bedfileMito.RDS")
...
message("Make sure that the name of the directory is the ID of the patient")
dirPrinc <- rstudioapi::selectDirectory()
id<- tail(strsplit(dirPrinc, "/")[[1]], n = 1)

if(a=="AC"|a=="ac"){
  ControlDB<-AddControl(ControlDB, PatientsDB, id, bed, c, include.chr, dirPrinc)

}else if (a=="AP"|a=="ap"){
  PatientsDB<-AddPatient(ControlDB, PatientsDB, id, bed, p, include.chr, dirPrinc)
  ...
  outAnalisis<-AnalyzePatient(ControlDB, PatientsDB, id, bed, p, include.chr, dirPrinc)
  ...
  Report(CNV_calls, graph=graph_, id, dir_patient=dirPrinc)

  message("The analisis has finished! Your will find the report on yourpatient's
  folder and the 3 databases(Patients, Control and Exons) on DataBases folder")
}else{
